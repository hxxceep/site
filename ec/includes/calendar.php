	...
	class calendar extends calendarExtensions
	{
		...
		*/
		public function updates($post, $file, $id_field = '')
		{
			...
			$this->update_insert_to_salary($post['description'],0,$start,	 $post['id']);
		...
		*/
		public function addEvent($post, $file='')
		{
			...
			if(isset($post) && is_array($post))
			{
				...
				$this->update_insert_to_salary($post['description'],0,$start,	 $inserted_id);
		...
		*/
		...
		//	$results = $this->categories;
			$results = array();
									mysqli_query($this->connection, 'SET character_set_connection=utf8');
		  $query =		mysqli_query($this->connection, "select company_name from company");
			...
			while($row = mysqli_fetch_array($query)){
			     array_push($results,$row["company_name"]);
			}
			//print_r($results);
